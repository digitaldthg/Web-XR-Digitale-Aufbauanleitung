...
class MainScene{
  ...
  PlayActionByName(clipName){
    ...
    //sets position and target of controls
      this.webXRScene.Controls.SetPosition(maskCameraPos.x,maskCameraPos.y,maskCameraPos.z);
      this.webXRScene.Controls.SetTarget(maskTargetPos.x,maskTargetPos.y,maskTargetPos.z, 2);

      //update visual sphere
      this.sphere.position.set(maskTargetPos.x,maskTargetPos.y,maskTargetPos.z);
  ...
  ChangeAnimationStep(dir){
    ...
    this.PlayActionByName(this.steps[nextStep]);

    console.log(this.steps[nextStep] , dir, nextStep);
  }
